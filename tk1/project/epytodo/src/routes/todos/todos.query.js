...
module.exports.update_todo = function (todo, callback) {
    var query = "UPDATE FROM todo SET title = ?, description = ?, due_time = ?, user_id = ?, status = ? WHERE id = ?";
    db.execute(
        query,
        [todo.title, todo.description, todo.due_time, todo.user_id, todo.status, todo.id],
        function (err, results, fields) {
            if (err) {
                console.log(err);
            }
