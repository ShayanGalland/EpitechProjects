...
bool Droid::operator()(const std::string* task, size_t expNeeded)
{
    bool haveExpRequired = _battleData->getExp() >= expNeeded;
    bool isEnergyRequired = _energy > 10;
    _energy = _energy - 10;

    if(isEnergyRequired) {
        if(haveExpRequired) {
            (*_battleData) += expNeeded/2;
            setStatus(new std::string((*task)));
            _status->append(" - Completed!");
            return true;
        }
        else {
            (*_battleData) += expNeeded;
            setStatus(new std::string((*task)));
            _status->append(" - Failed!");
            return false;
        }
    }
    else {
